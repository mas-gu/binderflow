...
def run_pocketxmol(protein_pdb, center, bbox_size, n_mols, out_dir,
    ...
    # Scale the pocket-conditioning radius with the bbox so wider
    # cross-chain interfaces aren't truncated. Lower bound 15 Å keeps
    # tight single-chain pockets behaving as before.
    radius = max(15.0, round(bbox_size * 0.75, 1))
    if radius > 15.0:
        log(f"PocketXMol: generating {n_mols} molecules "
            f"(SBDD + AR refinement, radius={radius} Å for bbox {bbox_size:.1f} Å)")
    else:
...
def plot_mol_dashboard(all_ranked, out_path, title="SBDD Pipeline"):
    ...
    try:
        ...
        bp = ax1.boxplot(data_bp, labels=tools_present, patch_artist=True)
...
def copy_top_molecules(ranked, out_dir, n=50, target_pdb=None, pocket_pdb=None,
    ...
    ``site_pairs`` is a list of (chain, resnum) tuples covering all chains
    of the binding site (multi-chain pockets supported).
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
...
def _write_molecule_pml(out_dir, copied_sdfs, target_name, pocket_name,
    ...
    ``site_pairs`` is a list of (chain, resnum) tuples; the selection is
    a per-chain union so multi-chain interface sites display correctly.
    """
    tool_colors = {
        "pocketflow":  "cyan",
        "molcraft":    "purple",
    ...
    # Build binding site selection: per-chain union for multi-chain sites
    site_sel = ""
    if site_pairs:
        by_chain = {}
        for c, r in site_pairs:
            by_chain.setdefault(c, []).append(r)
        parts = [f"(chain {c} and resi {'+'.join(str(r) for r in sorted(rs))})"
                 for c, rs in by_chain.items()]
        site_sel = " or ".join(parts)
    ...
    if target_name:
        ...
        # Highlight binding site in red. Wrap site_sel in parens so
        # multi-chain "A or B" selections compose correctly with the
        # outer "tobj and (...)".
        if site_sel:
...
""")
    ...
    if is_multichain:
        ...
        log(f"Binding site:     {args.site} ({len(site_pairs)} residues, chain {site_chains[0]})")
    ...
    if not args.dry_run:
        ...
        # Cap bbox to keep the de novo search space focused.
        # Single-chain pockets: 20 Å (larger boxes spread mols too thin).
        # Multi-chain (interface) pockets: 25 Å — common cleft widths
        # often exceed 20 Å, and the corrected centroid lands in-cleft.
        bbox_cap = 25.0 if is_multichain else 20.0
        if bbox_size > bbox_cap:
            log(f"  Note: bbox {bbox_size:.1f} Å capped to {bbox_cap:.1f} Å"
                f" ({'multi-chain' if is_multichain else 'single-chain'})")
            bbox_size = bbox_cap
        ...
        info = {
            "center": list(center),
            "bbox_size": bbox_size,
            "site_pairs": [list(p) for p in site_pairs],
            "chains": site_chains,
            "residues_per_chain": {
                c: sorted(r for cc, r in site_pairs if cc == c)
                for c in site_chains
            },
        }
